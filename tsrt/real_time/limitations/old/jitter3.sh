...
while [ "$a" -lt $5 ]
do
ssh root@$1 chrt $mysched $distprio taskset $mycpu $P/disturber50&
sleep 0.2
a=`expr $a + 1`;
done
...
./display-histogram.sh histo.txt "timer 1kHz running on CPU #$4 with scheduler: $2 , priority $myprio,  $5 disturber(s) with priority $distprio"
